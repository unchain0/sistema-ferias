...
test('professionals list displays correctly', async ({ page }) => {
  ...
  const professionalItems = page.locator(
    '[data-testid="professional-item"], .professional-item, .professional-card, tr[data-row-key], .table-row:not(:first-child)',
  );
  await expect(professionalItems.first())
    .toBeVisible()
    .catch(async () => {
      // If specific selectors don't exist, just check for general content
      await expect(page.locator('text=/profissionais|professionals/i').first()).toContainText(
        /profissionais|professionals/i,
      );
    });
});
...
test('add new professional functionality', async ({ page }) => {
  ...
  const addProfessionalButton = page.getByRole('button', {
    name: /adicionar|adicionar profissional|new|\+|create/i,
  });
